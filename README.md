...
| 复审回告闭环 | 流程说明 | 主力档（RES/LEG）归档即入待审池；六阶段状态机逐段机械推进（审查替身只见结构化审查包，异议方陈述须在 hearing 收窗，出口三选未裁不可回告，回告未齐不可闭环）——全部服务端强制（ADR-0010） |
...
| `pc_review` | 主席台 | 复审回告闭环（ADR-0010）：登记复审意见(request)／待审池出审(dispatch)／听证收窗／出口三选(adjudicate)／逐条回告(reply)／落地关联(link) |
...
pnpm run check     # typecheck + build + smoke（46 项端到端断言）+ 客户端渲染冒烟
```
...
- **1.6.0** — 复审回告闭环（ADR-0010，协议 v2.12）：已归档案卷六阶段复审（登记→受理→审查→沟通→出口→回告）；「有备必审」分层入池（RES/LEG 主力自动出审、降级态优先，CON/PLA/STR 用户触发，MIN 不复审）；**去 AI 中介**——审查替身/监督替身均由服务层 spawn（`ctx.agents.create`，seed 空、setup 硬编码），审查替身只见结构化审查包；异议方（反对票成员）原会话被动陈述；出口用户三选（修订新卷/解释原卷/驳回回告）；逐条回告闭环（立法法 §113 同构）；新增 `pc_review`/`pc_review_verdict`/`pc_review_statement`；监督替身派发从主持人迁至 tally 门二受阻服务层自动。
