...
| 复审回告闭环 | 流程说明 | 主力档（RES/LEG）归档即入池＋归档位点批量泵自动出审（按优先级，征询采信优先）；次级/弱档用户在场 request/dispatch 专项开启；降级标记唯一=征询采信；六阶段状态机逐段机械推进（审查替身只见结构化审查包，异议方陈述须在 hearing 收窗，出口三选未裁不可回告，回告未齐不可闭环）——全部服务端强制（ADR-0010） |
...
| `pc_review` | 主席台 | 复审回告闭环（ADR-0010）：登记复审意见(request)／专项出审(dispatch docIds)／听证收窗／出口三选(adjudicate)／分级批量驳回(batch-dismiss)／逐条回告(reply)／落地关联(link)；主力档归档由服务层批量泵自动出审 |
