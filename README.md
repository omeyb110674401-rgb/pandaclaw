...
| 监督窗口双门 | 流程说明 | ⭐阶段计票前须先有 `warning` 关窗预告＋`supervision` 监督记录，否则 `pc_tally` 直接拒绝（ADR-0008/0009） |
...
| `pc_supervise` | 监督替身 | 缺席用户的会中监督代言（ADR-0009）：提监督质疑、不算票、用户可撤回 |
...
浏览器  conversation.view「会议舞台」页签
        ...
        监督窗口四态(开放/即将关闭/二阶段替身/已关窗) · 计票历史(全轮次·征询标注)
        记录流(种类筛选/退回理由) · 复审子通道入口 · 空态快速开始卡
          ▲ faceOf('pandaclaw') 快照推送（客户端零折叠）
主机    PandaClawService ──自有记录域 pandaclaw(meetings/records/tallies)
        ├ 主席台 7 工具(普通会话) ├ 成员 2 工具(子代理作用域) └ 监督替身 1 工具(专用 preset)
