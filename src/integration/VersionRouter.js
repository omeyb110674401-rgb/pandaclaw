 ...
 * 版本路由器
 */
class VersionRouter extends events_1.EventEmitter {
    constructor(options = {}) {
        super();
        this.stats = {
            routedCount: 0,
    ...
    checkVersion(version) {
        ...
        if (version > CURRENT_VERSION) {
            ...
            const compatibility = changelog?.breakingChanges.length > 0 ? 'none' : 'partial';
            return {
                version: version,
                compatibility,
                warnings,
    ...
    // ==================== 内部处理器 ====================
    async _handleV1(message) {
        // v1 基础处理
        return { ...message, routedVersion: 1 };
    }
    async _handleV2(message) {
        // v2 向后兼容 v1，添加新特性支持
        // 如果是 v1 消息，自动升级
